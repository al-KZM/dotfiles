...
$CONFIG_DIR/lf/image.sh clear

# Calculate where the image should be placed on the screen.
#cols length of the preview window --> x
num=$(printf "%0.f\n" "`echo "$(tput cols) / 2" | bc`")
# start of the preview window (in cols) --> Max width
numb=$(printf "%0.f\n" "`echo "$(tput cols) - $num - 1" | bc`")
# mid line of the window --> Max height
...
case "$1" in
	...
	*.epub)
		...
		$CONFIG_DIR/lf/image.sh draw "$CACHE" $num 1 $numb $numc
		;;
	*.bmp|*.jpg|*.jpeg|*.png|*.xpm)
		$CONFIG_DIR/lf/image.sh draw "$1" $num 1 $numb $numc
